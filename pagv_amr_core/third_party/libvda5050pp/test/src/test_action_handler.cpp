...
void TestActionHandler::reset() noexcept(false) { this->reset_called_.set_value(); }

std::future<ActionCallbackFutures> TestActionHandler::getPrepareFuture() noexcept(true) {
  return this->prepare_called_.get_future();
}

std::future<void> TestActionHandler::getResetFuture() noexcept(true) {
  return this->reset_called_.get_future();
}

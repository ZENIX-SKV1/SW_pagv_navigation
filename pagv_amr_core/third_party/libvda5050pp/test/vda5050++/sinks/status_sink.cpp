...
TEST_CASE("sinks::StatusSink", "[events][sinks]") {
  ...
  testEvent<vda5050pp::events::SafetyStateSet>([sink] { sink.setSafetyState({}); });
  testSyncEvent<vda5050pp::events::SafetyStateGet>([sink]() { sink.getSafetyState(); });
  testEvent<vda5050pp::events::SafetyStateAlter>(
      [sink]() { sink.alterSafetyState([](auto &) { /* NOP */ }); });

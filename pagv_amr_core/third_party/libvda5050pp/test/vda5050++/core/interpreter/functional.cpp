...
static std::function<void(std::shared_ptr<vda5050pp::core::events::InterpreterEvent>)>
assertGraphExtensionEvent(uint32_t base_beg, uint32_t base_end, uint32_t horz_beg,
                          uint32_t horz_end) {
...
static std::function<void(std::shared_ptr<vda5050pp::core::events::InterpreterEvent>)>
assertGraphHorizonReplacementEvent(uint32_t horz_beg, uint32_t horz_end) {
  return [horz_beg, horz_end](std::shared_ptr<vda5050pp::core::events::InterpreterEvent> event) {
    THEN("The Event is a YieldGraphReplacementEvent") {
      REQUIRE(event->getId() ==
              vda5050pp::core::events::InterpreterEventType::k_yield_graph_replacement);
    }
...
TEST_CASE("core::interpreter - single node order (without action)", "[core][interpreter]") {
  using namespace test::data;
  auto test_order = test::data::mkTemplateOrder({
      TemplateElement{"n0", 0, true, {}},
      TemplateElement{"e0", 1, false, {}},
      TemplateElement{"n1", 2, false, {}},
  });

  auto test_order2 = test::data::mkTemplateOrder({
      TemplateElement{"n0", 0, true, {}},
  });

  std::list test_order_event_assertions{
      assertActionClearEvent(),
      assertGraphReplacementEvent(0, 0, 1, 2),
  };

  std::list test_order_event_assertions2{
      assertActionClearEvent(),
      assertGraphReplacementEvent(0, 0),
  };

  WHEN("Interpreting") {
    THEN("All events occur as expected (with horizon)") {
      testInterpreter(test_order, test_order_event_assertions);
    }
    THEN("All events occur as expected (without horizon)") {
      testInterpreter(test_order2, test_order_event_assertions2);
    }
  }
}

TEST_CASE("core::interpreter - only horizon", "[core][interpreter]") {
  using namespace test::data;
  auto test_order = test::data::mkTemplateOrder({
      TemplateElement{"n0", 0, false, {}},
      TemplateElement{"e0", 1, false, {}},
      TemplateElement{"n1", 2, false, {}},
  });
  ...
  std::list test_order_event_assertions{
      assertActionClearEvent(),
      assertGraphHorizonReplacementEvent(0, 2),
  };
...
TEST_CASE("core::interpreter - only horizon update", "[core][interpreter]") {
  using namespace test::data;

  auto test_order_2 = test::data::mkTemplateOrder({
      TemplateElement{"n1", 2, true, {}},
      TemplateElement{"e1", 3, false, {}},
      TemplateElement{"n2", 4, false, {}},
      TemplateElement{"e2", 5, false, {}},
      TemplateElement{"n3", 6, false, {}},
  });
  test_order_2.orderUpdateId = 1;

  std::list test_order_event_assertions{
      assertGraphExtensionEvent(2, 2, 3, 6),
  };

  WHEN("Interpreting") {
    THEN("All events occur as expected") {
      testInterpreter(test_order_2, test_order_event_assertions);
    }
  }
}
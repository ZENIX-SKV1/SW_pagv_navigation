...
class TestQueryNodeReachableHandle : public vda5050pp::handler::BaseQueryHandler {
private:
  bool reached_called_ = false;
  std::list<vda5050::Error> result_;

public:
  TestQueryNodeReachableHandle() {
    vda5050::Error err;
    err.errorType = "test";
    result_.emplace_back(err);
  }

  bool reachedCalled() const { return reached_called_; }

  std::list<vda5050::Error> getResult() const { return result_; }

  void reset() { reached_called_ = false; }

  std::list<vda5050::Error> queryNodeTriviallyReachable(
      std::shared_ptr<const vda5050::Node>) override {
    this->reached_called_ = true;
    return result_;
  }
};
...
TEST_CASE("core::agv_handler:QueryEventHandler", "[core][agv_handler]") {
  ...
  cfg.refQueryEventHandlerSubConfig().setAllowNodeTriviallyReachableDeviationCheck(false);
  ...
  auto query_node_reachable = std::make_shared<vda5050pp::events::QueryNodeTriviallyReachable>();
  auto node = std::make_shared<vda5050::Node>();
  node->nodeId = "node1";
  node->nodePosition = vda5050::NodePosition{0, 0, std::nullopt, 1, 1, "", std::nullopt};
  query_node_reachable->node = node;
  ...
  WHEN("The defaults are set a nop handler is set") {
    ...
    THEN("Node reachable events checked with the default behavior (last node matches by id)") {
      instance->getOrderManager().setAGVLastNodeId("node1");
      instance->getQueryEventManager().dispatch(query_node_reachable);
      auto future = query_node_reachable->getFuture();
      REQUIRE(future.wait_for(0s) == std::future_status::ready);
      REQUIRE(future.get().empty());
    }

    THEN("Node reachable events checked with the default behavior (last node does not match by "
         "id)") {
      instance->getOrderManager().setAGVLastNodeId("node2");
      instance->getQueryEventManager().dispatch(query_node_reachable);
      auto future = query_node_reachable->getFuture();
      REQUIRE(future.wait_for(0s) == std::future_status::ready);
  ...
  WHEN("Allowing deviation based checks") {
    vda5050pp::core::Instance::reset();
    ...
    THEN("Node reachable events checked with the default behavior (last node does not match by "
         "id and position mismatch)") {
      instance->getStatusManager().setAGVPosition(
          vda5050::AGVPosition{true, std::nullopt, std::nullopt, 10, 10, 0, "", std::nullopt});
      instance->getOrderManager().setAGVLastNodeId("node2");
      instance->getQueryEventManager().dispatch(query_node_reachable);
      auto future = query_node_reachable->getFuture();
      REQUIRE(future.wait_for(0s) == std::future_status::ready);
  ...
  WHEN("A node reachable handler is set") {
    vda5050pp::core::Instance::reset();
    auto instance = vda5050pp::core::Instance::init(cfg, evt_opts).lock();
    auto handler = std::make_shared<TestQueryNodeReachableHandle>();
    instance->setQueryHandler(handler);

...
public:
  ...
  void reset() override { this->reset_called_ = true; }
...
TEST_CASE("core::agv_handler::ActionEventHandler", "[core][agv_handler]") {
  ...
  WHEN("A reset event is sent") {
    auto evt = std::make_shared<vda5050pp::events::ActionReset>();
    instance->getActionEventManager().dispatch(evt);

    THEN("The action state is reset") { REQUIRE(handler->isResetCalled()); }
  }

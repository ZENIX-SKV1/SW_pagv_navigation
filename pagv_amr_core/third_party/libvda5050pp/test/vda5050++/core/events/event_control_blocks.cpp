...
TEST_CASE("core::events::EventControlParallel behaviour", "[core][events]") {
  vda5050pp::core::events::EventManagerOptions opts;
  opts.synchronous_event_dispatch = true;
  int teardown_called = 0;
  ...
  events::EventControlParallel parallel;
  parallel.add(latch_1);
  parallel.add(latch_2);
  parallel.add(latch_3);
  parallel.setTeardown([&teardown_called] { teardown_called++; });

  WHEN("The parallel block is not enabled and any event is dispatched") {
    mgr.dispatch(std::make_shared<Event1>());
    mgr.dispatch(std::make_shared<Event2>());
  ...
  WHEN("The parallel block is enabled") {
    parallel.enable();
    ...
    WHEN("Event1 is dispatched") {
      ...
      THEN("Teardown of the parallel block was not called") { REQUIRE(teardown_called == 0); }
      WHEN("Event2 is dispatched") {
        mgr.dispatch(std::make_shared<Event2>());

        THEN("The first and second latch are done") {
          REQUIRE(latch_1->getDoneCalled() == 1);
          REQUIRE(latch_2->getDoneCalled() == 1);
          REQUIRE(latch_3->getDoneCalled() == 0);
        }
        ...
        WHEN("Event3 is dispatched") {
          ...
          THEN("All latches are done") {
            REQUIRE(latch_1->getDoneCalled() == 1);
            REQUIRE(latch_2->getDoneCalled() == 1);
            REQUIRE(latch_3->getDoneCalled() == 1);
          }
          THEN("Teardown of the parallel block was called") { REQUIRE(teardown_called == 1); }
          WHEN("Any Event is dispatched") {
            mgr.dispatch(std::make_shared<Event1>());
            mgr.dispatch(std::make_shared<Event2>());
            mgr.dispatch(std::make_shared<Event3>());

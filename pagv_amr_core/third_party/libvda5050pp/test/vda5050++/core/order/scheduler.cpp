  ...
  WHEN("The first step is enqueued") {
    {
      ...
      WHEN("Resetting in active") {
        auto evt_navigation_reset =
            EventAsserter<vda5050pp::events::NavigationReset>::forNavigationEvent(
                [](auto) { return true; });

        scheduler.reset();

        evt_navigation_reset.assertNow([](auto) { return true; });
      }
    }
    WHEN("Reaching the first node") {
      {
        ...
        WHEN("Resetting in idle") {
          ...
          evt_navigation_reset.assertNone();
        }
      }
    }
...
TEST_CASE("core::order::Scheduler - only navigation with actions", "[core][order]") {
  vda5050pp::core::Instance::reset();
  vda5050pp::Config cfg;
  vda5050pp::core::events::EventManagerOptions opts;
  opts.synchronous_event_dispatch = true;
  cfg.refGlobalConfig().useWhiteList();
  vda5050pp::core::Instance::init(cfg, opts);
  ...
  WHEN("The first node is reached") {
    {
      ...
      WHEN("Resetting in active") {
        auto evt_action_reset = EventAsserter<vda5050pp::events::ActionReset>::forActionEvent(
            [](auto) { return true; });

        scheduler.reset();

        evt_action_reset.assertNow([](auto) { return true; });
      }
    }
...
TEST_CASE("core::order::Scheduler - pause/resume", "[core][order]") {
  ...
  WHEN("No pause was requested and all tasks pause") {
    {
      auto evt_order_status = EventAsserter<vda5050pp::core::events::OrderStatus>::forOrderEvent(
          [](auto) { return true; });
    ...
    WHEN("All tasks are resumed") {
      auto evt_order_status = EventAsserter<vda5050pp::core::events::OrderStatus>::forOrderEvent(
          [](auto) { return true; });

      scheduler.actionTransition("a11", vda5050pp::core::order::ActionTransition::isRunning());
      scheduler.actionTransition("a12", vda5050pp::core::order::ActionTransition::isRunning());
      scheduler.navigationTransition(vda5050pp::core::order::NavigationTransition::isResumed());
          ...
          WHEN("Resuming") {
            {
              ...
              WHEN("Resetting in resuming") {
                auto evt_navigation_reset =
                    EventAsserter<vda5050pp::events::NavigationReset>::forNavigationEvent(
                        [](auto) { return true; });
                auto evt_action_reset =
                    EventAsserter<vda5050pp::events::ActionReset>::forActionEvent(
                        [](auto) { return true; });
...
TEST_CASE("core::order::Scheduler - cancel", "[core][order]") {
  ...
  WHEN("The tasks are paused") {
    ...
    WHEN("A cancel request is started") {
      ...
      assertOrderStatus(evt_order_status, vda5050pp::misc::OrderStatus::k_order_canceling_paused);
  ...
  WHEN("Interrupting with a HARD action") {
    {
      ...
      WHEN("Resetting in interrupting") {
        auto evt_navigation_reset =
            EventAsserter<vda5050pp::events::NavigationReset>::forNavigationEvent(
                [](auto) { return true; });
        auto evt_action_reset = EventAsserter<vda5050pp::events::ActionReset>::forActionEvent(
    ...
    WHEN("actions and navigation pause/finish/failed") {
      ...
      WHEN("i1 finishes") {
        ...
        WHEN("resuming the paused navigation") {
          {
            auto evt_order_status =
                EventAsserter<vda5050pp::core::events::OrderStatus>::forOrderEvent(
                    [](auto) { return true; });

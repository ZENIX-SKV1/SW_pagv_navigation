...
TEST_CASE("core::checks::checkOrderAppend", "[core][checks]") {
  ...
  WHEN("There is no current graph") {
    WHEN("An appending order is checked") {
      ...
      THEN("An error is returned") {
        auto ret = vda5050pp::core::checks::checkOrderAppend(order);
  ...
  WHEN("There is a current graph") {
    ...
    WHEN("The order is active") {
      ...
      WHEN("A extending order with non-matching first node is checked") {
        vda5050::Order order;
        order.orderId = "order";
        order.orderUpdateId = 1;
        vda5050::Node node;
    ...
    WHEN("The order is idle") {
      instance->getOrderManager().setAGVLastNode(2);

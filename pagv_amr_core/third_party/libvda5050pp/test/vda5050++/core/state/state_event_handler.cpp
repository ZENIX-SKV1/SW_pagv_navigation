...
inline void quickStateEventTest(EventT &&evt, CheckT &&check_fn,
  ...
  WHEN(fmt::format("Dispatching a {}", typeid(EventT).name())) {
    ...
    if constexpr (std::is_base_of_v<vda5050pp::core::events::MessageEvent,
                                    typename std::decay_t<EventT>::element_type>) {
      instance.getMessageEventManager().dispatch(std::forward<EventT>(evt));
    } else {
...
template <typename Event, typename CheckT>
inline void quickGetTest(Event &&evt, CheckT &&check_fn) {
  static_assert(vda5050pp::core::common::is_signature_v<
                    CheckT, void(const typename std::decay_t<Event>::element_type::result_type &)>,
                "CheckT must be a function with signature void(const Event::result_type &)");

  WHEN(fmt::format("Dispatching a {}", typeid(Event).name())) {
    auto future = evt->getFuture();

    vda5050pp::core::Instance::ref().getStatusEventManager().dispatch(std::forward<Event>(evt));

    THEN("The future is ready") {
...
TEST_CASE("core::state::StateEventHandler", "[core][state]") {
  ...
  WHEN("The order manager contains an order") {
    ...
    WHEN("A YieldGraphExtension event without a horizon is dispatched") {
      ...
      std::promise<std::pair<std::list<std::shared_ptr<const vda5050::Node>>,
                             std::list<std::shared_ptr<const vda5050::Edge>>>>
          promise_horizon;
      auto future_horizon = promise_horizon.get_future();
      ...
      sub.subscribe([&promise_horizon](
                        std::shared_ptr<vda5050pp::events::NavigationHorizonUpdate> evt) {
        promise_horizon.set_value(std::make_pair(evt->horz_replace_nodes, evt->horz_replace_edges));
      });
      ...
      THEN("An empty horizon event was dispatched") {
        REQUIRE(future_horizon.wait_for(100ms) == std::future_status::ready);
        auto [nodes, edges] = future_horizon.get();
        REQUIRE(nodes.empty());
        REQUIRE(edges.empty());
      }
    }

    WHEN("A YieldGraphExtension event with a horizon is dispatched") {
      auto sub = instance->getNavigationEventManager().getScopedNavigationEventSubscriber();
      std::promise<std::pair<std::list<std::shared_ptr<const vda5050::Node>>,
                             std::list<std::shared_ptr<const vda5050::Edge>>>>
          promise;
      ...
      evt->graph = wrap_shared(vda5050pp::core::state::Graph({
          ...
          vda5050pp::core::state::GraphElement(wrap_shared(mkEdge("e1", 1, true, {}))),
          vda5050pp::core::state::GraphElement(wrap_shared(mkNode("n2", 2, true, {}))),
          vda5050pp::core::state::GraphElement(wrap_shared(mkEdge("e2", 3, false, {}))),
          vda5050pp::core::state::GraphElement(wrap_shared(mkNode("n3", 4, false, {}))),
      }));
      evt->order_update_id = 1;
      ...
      THEN("A corresponding horizon update event was dispatched") {
        REQUIRE(future_horizon.wait_for(0s) == std::future_status::ready);
        auto [nodes, edges] = future_horizon.get();
        REQUIRE(nodes.size() == 1);
        REQUIRE(edges.size() == 1);
        REQUIRE(nodes.front()->nodeId == "n3");
        REQUIRE(edges.front()->edgeId == "e2");
      }
    }

    WHEN("A YieldGraphReplacement event without horizon is dispatched") {
      auto sub = instance->getNavigationEventManager().getScopedNavigationEventSubscriber();
      std::promise<std::pair<std::list<std::shared_ptr<const vda5050::Node>>,
                             std::list<std::shared_ptr<const vda5050::Edge>>>>
          promise;
      ...
      std::promise<std::string> promise_last_node_id;
      auto future_last_node_id = promise_last_node_id.get_future();
      auto sub_order = instance->getAgvOrderEventManager().getScopedOrderEventSubscriber();
      sub_order->subscribe(
          [&promise_last_node_id](std::shared_ptr<vda5050pp::events::OrderLastNodeChanged> evt) {
            promise_last_node_id.set_value(evt->last_node_id);
          });

      // Add canary error
      instance->getStatusManager().addError(vda5050::Error{});

      auto evt = std::make_shared<vda5050pp::core::events::YieldGraphReplacement>();
      evt->graph = wrap_shared(vda5050pp::core::state::Graph({
          vda5050pp::core::state::GraphElement(wrap_shared(mkNode("temp-node", 0, true, {}))),
          vda5050pp::core::state::GraphElement(wrap_shared(mkEdge("e1", 1, true, {}))),
          vda5050pp::core::state::GraphElement(wrap_shared(mkNode("n2", 2, true, {}))),
      }));
      evt->order_id = "order2";
      ...
      THEN("The lastNodeId is matching the first node of the replacement graph") {
        REQUIRE(future_last_node_id.wait_for(0s) == std::future_status::ready);
        REQUIRE(future_last_node_id.get() == "temp-node");
      }

      THEN("Errors are cleared") { REQUIRE(instance->getStatusManager().getErrors().empty()); }
    ...
    WHEN("A YieldGraphReplacement event with horizon is dispatched") {
      auto sub = instance->getNavigationEventManager().getScopedNavigationEventSubscriber();
      std::promise<std::pair<std::list<std::shared_ptr<const vda5050::Node>>,
                             std::list<std::shared_ptr<const vda5050::Edge>>>>
          promise;
      ...
      THEN("The corresponding horizon update is dispatched") {
        REQUIRE(future.wait_for(0s) == std::future_status::ready);
        auto [nodes, edges] = future_horizon.get();
        REQUIRE(nodes.size() == 1);
        REQUIRE(edges.size() == 1);
  ...
  WHEN("A ClearAfterReset is dispatched") {
    auto evt_clear = std::make_shared<vda5050pp::core::events::OrderClearAfterReset>();
    instance->getOrderEventManager().dispatch(evt_clear);
    THEN("The OrderManager has no actions") {
      REQUIRE_THROWS_AS(instance->getOrderManager().getAction("action1"),
                        vda5050pp::VDA5050PPInvalidArgument);
    }
    THEN("The OrderManager has no graph") { REQUIRE_FALSE(instance->getOrderManager().hasGraph()); }
  }

  WHEN("A ClearAfterCancel is dispatched") {
    auto evt_clear = std::make_shared<vda5050pp::core::events::OrderClearAfterCancel>();
    evt_clear->cancel_action = std::make_shared<vda5050::Action>();
    instance->getOrderEventManager().dispatch(evt_clear);
    THEN("The OrderManager has no actions") {
      REQUIRE_THROWS_AS(instance->getOrderManager().getAction("action1"),
                        vda5050pp::VDA5050PPInvalidArgument);
  ...
  quickStateEventTest(
      ...
      [](const vda5050::State &state) {
        ...
        auto loads_get = std::make_shared<vda5050pp::events::LoadsGet>();
        quickGetTest(loads_get,
                     [](const std::vector<vda5050::Load> &l) { REQUIRE(l.size() == 1); });
  ...
  quickStateEventTest(
      ...
      [](const vda5050::State &s) {
        ...
        auto operating_mode_get = std::make_shared<vda5050pp::events::OperatingModeGet>();
        quickGetTest(operating_mode_get, [](const vda5050::OperatingMode &o) {
          REQUIRE(o == vda5050::OperatingMode::TEACHIN);
        });
  ...
  quickStateEventTest(
      ...
      [](const vda5050::State &s) {
        ...
        auto battery_state_get = std::make_shared<vda5050pp::events::BatteryStateGet>();
        quickGetTest(battery_state_get, [](const vda5050::BatteryState &s2) {
          REQUIRE(s2.batteryHealth.value() == 100);
        });
  ...
  auto safety_state_set = std::make_shared<vda5050pp::events::SafetyStateSet>();
  safety_state_set->safety_state.fieldViolation = true;

  quickStateEventTest(
      safety_state_set,
      [](const vda5050::State &s) {
        REQUIRE(s.safetyState.fieldViolation == true);

        auto safety_state_get = std::make_shared<vda5050pp::events::SafetyStateGet>();
        quickGetTest(safety_state_get,
                     [](const vda5050::SafetyState &s2) { REQUIRE(s2.fieldViolation == true); });

        auto request_new_base = std::make_shared<vda5050pp::events::RequestNewBase>();
        quickStateEventTest(
            request_new_base,
            [](const vda5050::State &state) { REQUIRE(state.newBaseRequest.value()); }, true);

        auto safety_state_alter = std::make_shared<vda5050pp::events::SafetyStateAlter>();
        safety_state_alter->alter_function = [](auto &ss) { ss.fieldViolation = false; };
        quickStateEventTest(
            safety_state_alter,
            [](const vda5050::State &s2) { REQUIRE(s2.safetyState.fieldViolation == false); },
            true);
      },
      true);
  ...
  quickStateEventTest(
      info_add,
      [](const vda5050::State &state) {
        REQUIRE(state.information.has_value());
        ...
        quickStateEventTest(
            info_alter,
            [](const vda5050::State &s) {
              REQUIRE(s.information.has_value());
              REQUIRE(s.information->size() == 2);
            },
            true);
      },
      true);

  auto message_error = std::make_shared<vda5050pp::core::events::MessageErrorEvent>();
  message_error->error_type = vda5050pp::misc::MessageErrorType::k_json_deserialization;
  quickStateEventTest(
      message_error, [](const vda5050::State &state) { REQUIRE(state.errors.size() == 1); }, true);
}

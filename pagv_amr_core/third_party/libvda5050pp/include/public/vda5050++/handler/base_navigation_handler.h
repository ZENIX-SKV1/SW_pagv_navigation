...
public:
  ...
  ///
  ///\brief Reset the navigation handler state.
  ///
  /// After returning from this function, the navigation handler should be in a state, where
  /// does not call any feedback-functions until receiving new goals.
  ///
  /// This function is currently only called, when the AGV changes it's operating mode to MANUAL.
  ///

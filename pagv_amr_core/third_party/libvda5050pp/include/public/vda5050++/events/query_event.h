...
///
///\brief THis event is dispatched by the library, for each retrieved order with orderUpdateId=0.
/// The AGV should determine if it can consider itself to be on the given node. This
/// behaviour is required to resume an order after canceling. (See VDA5050 6.6.1)
///
struct QueryNodeTriviallyReachable : public QueryEvent,
                                     SynchronizedEvent<std::list<vda5050::Error>> {

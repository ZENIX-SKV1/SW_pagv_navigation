...
///
///\brief This event can be dispatched by the user to set the current SafetyState.
///
struct SafetyStateSet : public StatusEventId<StatusEventType::k_safety_state_set> {
  vda5050::SafetyState safety_state;
};

///
///\brief This event can be dispatched by the user to obtain the current SafetyState.
/// The library will set the result, such that this->getFuture() will eventually contain the result.
///
struct SafetyStateGet : public StatusEventId<StatusEventType::k_safety_state_get>,
                        SynchronizedEvent<vda5050::SafetyState> {};

///
///\brief This event can be dispatched by the user to alter the SafetyState directly.
///
struct SafetyStateAlter : public StatusEventId<StatusEventType::k_safety_state_alter> {
  std::function<void(vda5050::SafetyState &)> alter_function;
};

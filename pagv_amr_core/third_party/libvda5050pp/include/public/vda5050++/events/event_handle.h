...
public:
  ...
  ///
  ///\brief Get a new ScopedOrderEventSubscriber.
  ///
  ///\return std::shared_ptr<ScopedOrderEventSubscriber>
  ///\throws VDA5050PPNotInitialized if (this->isValid() == false)
  ///
  std::shared_ptr<ScopedOrderEventSubscriber> getScopedOrderEventSubscriber() const noexcept(false);
  ...
  ///\throws VDA5050PPNotInitialized if (this->isValid() == false)
  ///
  void dispatch(std::shared_ptr<vda5050pp::events::NavigationStatusControl> data) const
      noexcept(false);

...
| `getControlSink`            | Return a new [`vda5050pp::sinks::ControlSink`](doxygen/html/classvda5050pp_1_1sinks_1_1ControlSink.html).                                                                                          |
...
Handlers can be registered instead of handling events by hand. 
For example the [`vda5050pp::handler::BaseNavigationHandler`](doxygen/html/classvda5050pp_1_1handler_1_1BaseNavigationHandler.html) wraps the handling and dispatching of
`NavigationEvent`s and `NavigationStatusEvent`s.

It is important to note, that event callbacks are processed by one thread (per event class).
This means, that handler functions must return before the next function of the handler can be called.
...
| `reset`                | The order was forcefully reset and the user-state of this handler has to be reset, too.  After returning from this function, it is expected, that all previously known actions are forgotten. |
...
| `reset`              | The order was forcefully reset and the user-state of this handler has to be reset, too.  After returning from this function, it is expected, that all previously known nodes/edges are forgotten. |
...
| `queryNodeTriviallyReachable` | Is a node "trivially" reachable (used for order accepting, see [VDA5050 6.6.1](https://github.com/VDA5050/VDA5050/blob/release/2.0.0/VDA5050_EN_V1.md#-661-concept-and-logic)) |
...
You only have to implement `vda5050pp::handler::BaseActionHandler::prepare` `vda5050pp::handler::BaseActionHandler::reset` and pass 
...
- the current base and horizon
- the errors set in the state

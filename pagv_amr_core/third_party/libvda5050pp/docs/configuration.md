...
### `[module.DistanceNodeHandler]` subtable

The DistanceNodeHandler sub config contains configuration for automatically calculating the distance to the last reached node.

| key                         | description                                     | optional | default |
| --------------------------- | ----------------------------------------------- | -------- | ------- |
| distance_interpolation_type | How to interpolate distances (0=none, 1=linear) | yes      | 0       |
...
| default_accept_zone_sets                       | The default set of zone sets, which will be accepted.                                            | yes      |
| allow_node_trivially_reachable_deviation_check | Allow the default QueryNodeTriviallyReachable check to use the deviation of the node.            | yes      |
| reachable_check_default_node_deviation_xy      | When using the deviation based check, use this value as a default for nodes without a deviation. | yes      |
| reachable_check_default_node_deviation_theta   | When using the deviation based check, use this value as a default for nodes without a deviation. | yes      |
| reachable_check_overwrite_node_deviation_xy    | When using the deviation based check, **always** use this value for nodes.                       | yes      |
| reachable_check_overwrite_node_deviation_theta | When using the deviation based check, **always** use this value for nodes.                       | yes      |

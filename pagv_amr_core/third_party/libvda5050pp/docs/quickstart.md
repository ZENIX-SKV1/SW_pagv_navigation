...
public:
  ...
  void reset() override {
    // The order was forcefully reset and the user-state of this handler has to be reset, too.
    // After returning from this function, it is expected, that all previously known nodes/edges
    // are forgotten.

    // This is currently only called, when the AGV's operating mode switched to MANUAL
    // (and reported to the library)
  }
};
```
...
public:
  ...
  void reset() override {
    // The order was forcefully reset and the user-state of this handler has to be reset, too.
    // After returning from this function, it is expected, that all previously known actions
    // are forgotten.
...
or control the current order with the [`vda5050pp::sinks::ControlSink`](doxygen/html/classvda5050pp_1_1sinks_1_1ControlSink.html).

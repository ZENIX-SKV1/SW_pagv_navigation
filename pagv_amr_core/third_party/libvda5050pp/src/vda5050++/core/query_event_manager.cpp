...
void ScopedQueryEventSubscriber::subscribe(
    std::function<void(std::shared_ptr<vda5050pp::events::QueryNodeTriviallyReachable>)>
        &&callback) noexcept(true) {
  this->remover_.appendListener(
      vda5050pp::events::QueryEventType::k_node_trivially_reachable,
      eventpp::argumentAdapter<void(
          std::shared_ptr<vda5050pp::events::QueryNodeTriviallyReachable>)>(std::move(callback)));
}

QueryEventManager::QueryEventManager(const vda5050pp::core::events::EventManagerOptions &opts)
    : opts_(opts),
...
void QueryEventManager::dispatch(
    std::shared_ptr<vda5050pp::events::QueryNodeTriviallyReachable> data,
    bool synchronous) noexcept(true) {
  if (this->opts_.synchronous_event_dispatch || synchronous) {
    this->query_event_queue_.dispatch(vda5050pp::events::QueryEventType::k_node_trivially_reachable,
                                      data);
  } else {
    this->query_event_queue_.enqueue(vda5050pp::events::QueryEventType::k_node_trivially_reachable,
                                     data);
  }
}

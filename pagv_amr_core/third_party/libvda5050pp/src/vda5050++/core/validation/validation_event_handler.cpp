...
inline std::list<vda5050::Error> queryFirstNodeTriviallyReachable(const vda5050::Node &node) {
  auto evt = std::make_shared<vda5050pp::events::QueryNodeTriviallyReachable>();
  evt->node = std::make_shared<vda5050::Node>(node);
  auto future = evt->getFuture();
  ...
  if (future.wait_for(1s) != std::future_status::ready) {
    throw vda5050pp::VDA5050PPSynchronizedEventTimedOut(MK_FN_EX_CONTEXT(
        fmt::format("QueryFirstNodeTriviallyReachable(node_id={}, node_seq={}) timed out.",
...
inline std::list<vda5050::Error> actionChecks(const vda5050::Order &order) {
  ...
  bool skip_first_node = order.orderUpdateId > 0;  // Skip first node, if the order appends
  for (const auto &node : order.nodes) {
    if (skip_first_node) {
      // Do not validate the action on the first node, since they are already validated (by the
      // previous order) and are verified to be the same
      skip_first_node = false;
      continue;
    }
    for (const auto &action : node.actions) {
      vda5050pp::core::getValidationLogger()->debug(
          "Sending ActionValidate(action={}, node) to AGV interface", action.actionId);
  ...
  for (const auto &edge : order.edges) {
    for (const auto &action : edge.actions) {
      vda5050pp::core::getValidationLogger()->debug(
          "Sending ActionValidate(action={}, edge) to AGV interface", action.actionId);
      auto v_evt = std::make_shared<vda5050pp::events::ActionValidate>();
      v_evt->action = std::make_shared<vda5050::Action>(action);
  ...
  for (auto &[_, action, future] : results) {
    if (future.wait_for(0s) != std::future_status::ready) {
      vda5050pp::core::getValidationLogger()->debug(
          "Future for action {} unavailable -> unknown action", action->actionType);
      ...
      vda5050pp::core::getValidationLogger()->debug("Got validation result for Action {}",
                                                    action->actionId);
...
void ValidationEventHandler::handleValidateOrder(
  ...
  if (evt->order->orderUpdateId == 0) {
    errors.splice(errors.end(), queryFirstNodeTriviallyReachable(*evt->order->nodes.begin()));
  }
  errors.splice(errors.end(), actionChecks(*evt->order));
...
void ValidationEventHandler::handleValidateInstantActions(
  ...
  for (const auto &action : evt->instant_actions->actions) {
    ...
    errors.splice(errors.end(), checks::uniqueActionId(action, seen_ids,
                                                       vda5050pp::misc::ActionContext::k_instant));
  ...
  for (auto &[_, action, future] : results) {
    ...
    if (future.wait_for(0s) != std::future_status::ready) {
      ...
      auto action_errors = future.get();
      if (!action_errors.empty()) {
        action_failed = true;
      }
      getValidationLogger()->debug("Got validation result for Action {}", action->actionId);
      errors.splice(errors.end(), std::move(action_errors));
    }

    // If the instant action failed, also set it failed inside the state.actionStates
    if (action_failed) {
      auto new_action_evt = std::make_shared<vda5050pp::core::events::YieldNewAction>();
      new_action_evt->action = action;

      auto fail_evt = std::make_shared<vda5050pp::core::events::OrderActionStatusChanged>();
      fail_evt->action_id = action->actionId;
      fail_evt->action_status = vda5050::ActionStatus::FAILED;

      Instance::ref().getInterpreterEventManager().synchronousDispatch(new_action_evt);
      Instance::ref().getOrderEventManager().synchronousDispatch(fail_evt);
    }
  }

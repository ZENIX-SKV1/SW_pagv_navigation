...
void OrderEventHandler::handleOperatingModeChanged(
    std::shared_ptr<vda5050pp::core::events::OperatingModeChangedEvent> evt) noexcept(false) {
  if (evt == nullptr) {
    throw vda5050pp::VDA5050PPInvalidEventData(MK_EX_CONTEXT(""));
  }

  if (evt->operating_mode == vda5050::OperatingMode::MANUAL) {
...
void OrderEventHandler::initialize(vda5050pp::core::Instance &instance) {
  ...
  this->state_event_subscriber_ = instance.getStateEventManager().getScopedSubscriber();
  ...
  this->state_event_subscriber_->subscribe<vda5050pp::core::events::OperatingModeChangedEvent>(
      std::bind(std::mem_fn(&OrderEventHandler::handleOperatingModeChanged), this,
                std::placeholders::_1));
}
...
void OrderEventHandler::deinitialize(vda5050pp::core::Instance &) {
  ...
  this->state_event_subscriber_.reset();
}

...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerIdle::reset() {
  return {std::make_unique<SchedulerIdle>(this->scheduler()), false};
}
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerIdlePaused::reset() {
  return {std::make_unique<SchedulerIdlePaused>(this->scheduler()), false};
}
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerActive::cancel() {
  ...
  if (auto nt = this->scheduler().getNavigationTask(); nt != nullptr && !nt->isTerminal()) {
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerActive::update() {
  ...
  // If there are no active tasks and no navigation task, go to idle
  ...
  // Otherwise, if there are no running tasks, go to paused (because the must be active tasks, which
  // are all paused). This allows the user to pause the order manually (by pausing all tasks)
  if (this->scheduler().getRunningActionTasksById().empty() &&
      (this->scheduler().getNavigationTask() == nullptr ||
       this->scheduler().getNavigationTask()->isPaused() ||
       this->scheduler().getNavigationTask()->isTerminal())) {
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerActive::reset() {
  this->scheduler().doReset();
  return {std::make_unique<SchedulerIdle>(this->scheduler(), true), false};
}
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerCanceling::update() {
  this->scheduler().updateTasks();
  this->scheduler().updateTasksInterruptMapping();
  this->scheduler().updateFetchNext(true);
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerCanceling::reset() {
  // TODO: This is an edge-case. The cancel instant action must be failed
  this->scheduler().doReset();
  return {std::make_unique<SchedulerIdle>(this->scheduler(), true), false};
}
...
// Scheduler Canceling Paused State
// ///////////////////////////////////////////////////////////////////////
SchedulerCancelingPaused::SchedulerCancelingPaused(Scheduler &scheduler, bool notify)
    : SchedulerStateID(scheduler) {
  if (notify) {
    auto evt = std::make_shared<vda5050pp::core::events::OrderStatus>();
    evt->status = vda5050pp::misc::OrderStatus::k_order_canceling_paused;
    Instance::ref().getOrderEventManager().dispatch(evt);
  }
}
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerCancelingPaused::cancel() {
  throw vda5050pp::VDA5050PPInvalidState(MK_EX_CONTEXT("Cannot cancel during canceling paused"));
}
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerCancelingPaused::pause() {
  throw vda5050pp::VDA5050PPInvalidState(MK_EX_CONTEXT("Cannot pause during canceling paused"));
}
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerCancelingPaused::resume() {
  throw vda5050pp::VDA5050PPInvalidState(MK_EX_CONTEXT("Cannot resume during canceling paused"));
}
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerCancelingPaused::interrupt() {
  throw vda5050pp::VDA5050PPInvalidState(MK_EX_CONTEXT("Cannot interrupt during canceling paused"));
}
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerCancelingPaused::update() {
  this->scheduler().updateTasks();
  this->scheduler().updateTasksInterruptMapping();
  ...
  return {std::make_unique<SchedulerCancelingPaused>(this->scheduler()), false};
}
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerCancelingPaused::reset() {
  // TODO: This is an edge-case. The cancel instant action must be failed
  this->scheduler().doReset();
  return {std::make_unique<SchedulerIdlePaused>(this->scheduler(), true), false};
}
std::string SchedulerCancelingPaused::describe() { return "SchedulerCancelingPaused"; }
////////////////////////////////////////////////////////////////////////////////////////////////////
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerResuming::reset() {
  this->scheduler().doReset();
  return {std::make_unique<SchedulerIdle>(this->scheduler(), true), false};
}
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerPausing::reset() {
  this->scheduler().doReset();
  return {std::make_unique<SchedulerIdle>(this->scheduler(), true), false};
}
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerPaused::cancel() {
  ...
  this->scheduler().clearQueues(false);

  return {std::make_unique<SchedulerCancelingPaused>(this->scheduler(), true), false};
}
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerPaused::update() {
  ...
  // When the user resumes all task, go to active
  if (this->scheduler().getPausedActionTasksById().empty() &&
      (this->scheduler().getNavigationTask() == nullptr ||
       !this->scheduler().getNavigationTask()->isPaused())) {
    return {std::make_unique<SchedulerActive>(this->scheduler(), true), false};
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerPaused::reset() {
  this->scheduler().doReset();
  return {std::make_unique<SchedulerIdlePaused>(this->scheduler(), true), false};
}
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerFailed::reset() {
  this->scheduler().doReset();
  return {std::make_unique<SchedulerIdle>(this->scheduler(), true), false};
}
...
std::pair<std::unique_ptr<SchedulerState>, bool> SchedulerInterrupting::reset() {
  this->scheduler().doReset();
  return {std::make_unique<SchedulerIdle>(this->scheduler(), true), false};
}
...
void Scheduler::doReset() {
  this->clearQueues(false);

  // Sync reset AGV
  auto reset_nav = std::make_shared<vda5050pp::events::NavigationReset>();
  Instance::ref().getNavigationEventManager().dispatch(reset_nav, true);
  auto reset_act = std::make_shared<vda5050pp::events::ActionReset>();
  Instance::ref().getActionEventManager().dispatch(reset_act, true);

  // Clear scheduler state
  this->active_action_tasks_by_id_.clear();
  this->paused_action_tasks_by_id_.clear();
  this->running_action_tasks_by_id_.clear();
  this->navigation_task_.reset();

  // Clear vda5050 state
  auto evt = std::make_shared<vda5050pp::core::events::OrderClearAfterReset>();
  Instance::ref().getOrderEventManager().dispatch(evt);
}
...
void Scheduler::updateFetchNext(bool no_actions) {
...
void Scheduler::updateFetchNext(
  ...
  // Action Groups may be executed in parallel if none of the action groups has blocking type HARD
  if (!this->active_action_tasks_by_id_.empty() &&
      (this->current_action_blocking_type_ == vda5050::BlockingType::HARD ||
       evt->blocking_type_ceiling == vda5050::BlockingType::HARD)) {
    return;
  }

  // Only allow actions that are non blocking if driving, but make sure to not trigger these actions
  // prematurely
  if (this->navigation_task_ != nullptr &&
      (evt->blocking_type_ceiling != vda5050::BlockingType::NONE ||
       Instance::ref().getOrderManager().getAGVLastNodeSequenceId() < evt->node_seq_id)) {
    return;
  }
...
void Scheduler::reset(std::optional<Lock> lock) {
  auto e_lock = this->ensureLock(std::move(lock));
  getOrderLogger()->debug("Scheduler::reset() pre_state_={}", this->state_->describe());
  auto [state, update] = this->state_->reset();
  this->state_ = std::move(state);
  getOrderLogger()->debug("Scheduler::reset() post_state_={}", this->state_->describe());

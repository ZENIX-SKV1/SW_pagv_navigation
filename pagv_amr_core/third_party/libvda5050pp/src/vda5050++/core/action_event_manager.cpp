...
void ScopedActionEventSubscriber::subscribe(
    std::function<void(std::shared_ptr<vda5050pp::events::ActionReset>)>
        &&callback) noexcept(true) {
  this->remover_.appendListener(
      vda5050pp::events::ActionEventType::k_action_reset,
      eventpp::argumentAdapter<void(std::shared_ptr<vda5050pp::events::ActionReset>)>(
          std::move(callback)));
}

ActionEventManager::ActionEventManager(const vda5050pp::core::events::EventManagerOptions &opts)
    : opts_(opts),
      thread_(
...
void ActionEventManager::dispatch(std::shared_ptr<vda5050pp::events::ActionReset> data,
                                  bool synchronous) noexcept(false) {
  if (this->opts_.synchronous_event_dispatch || synchronous) {
    this->action_event_queue_.dispatch(vda5050pp::events::ActionEventType::k_action_reset, data);
  } else {
    this->action_event_queue_.enqueue(vda5050pp::events::ActionEventType::k_action_reset, data);
  }
}

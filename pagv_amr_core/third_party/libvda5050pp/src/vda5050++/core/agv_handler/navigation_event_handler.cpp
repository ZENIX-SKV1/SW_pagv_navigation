...
void NavigationEventHandler::handleReset(
    std::shared_ptr<vda5050pp::events::NavigationReset> data) const noexcept(false) {
  if (data == nullptr) {
    throw vda5050pp::VDA5050PPInvalidEventData(MK_EX_CONTEXT("NavigationReset contains no data"));
  }

  getAGVHandlerLogger()->debug("NavigationHandler::handleReset()");

  if (auto ptr = Instance::ref().getNavigationHandler().lock(); ptr) {
    ptr->reset();
  } else {
    getAGVHandlerLogger()->debug("no navigation handler set");
  }
}
...
void NavigationEventHandler::initialize(vda5050pp::core::Instance &instance) {
  ...
  this->subscriber_->subscribe(
      std::bind(std::mem_fn(&NavigationEventHandler::handleReset), this, std::placeholders::_1));
}

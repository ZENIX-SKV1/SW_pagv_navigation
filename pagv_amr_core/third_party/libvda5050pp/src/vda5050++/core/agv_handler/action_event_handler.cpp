...
void ActionEventHandler::initialize(vda5050pp::core::Instance &instance) noexcept(false) {
  ...
  this->subscriber_->subscribe(
      std::bind(std::mem_fn(&ActionEventHandler::handleResetEvent), this, std::placeholders::_1));
}
...
void ActionEventHandler::handleResetEvent(
    std::shared_ptr<vda5050pp::events::ActionReset> data) noexcept(false) {
  if (data == nullptr) {
    throw vda5050pp::VDA5050PPInvalidEventData(MK_EX_CONTEXT("ActionReset contains no data"));
  }

  getAGVHandlerLogger()->debug("handleResetEvent()");

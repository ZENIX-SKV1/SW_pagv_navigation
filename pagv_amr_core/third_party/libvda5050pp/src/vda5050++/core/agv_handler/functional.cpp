...
bool vda5050pp::core::agv_handler::isOnNode(const vda5050::AGVPosition &agv,
  ...
  math::Circle<double> n_range{
      n_pos, override_deviation_xy.value_or(
                 node.allowedDeviationXY.value_or(default_deviation_xy.value_or(0)))};
  ...
  if (node.theta.has_value()) {
    ...
    if (std::abs(theta_diff) >
        override_deviation_theta.value_or(
            node.allowedDeviationTheta.value_or(default_deviation_theta.value_or(0.0)))) {
      return false;
    }
  }

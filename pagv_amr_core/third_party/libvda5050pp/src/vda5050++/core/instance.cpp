...
static const vda5050pp::core::AutoRegisterModule<vda5050pp::core::agv_handler::DistanceNodeHandler,
                                                 module_keys::k_distance_node_handler>
    _auto_register_distance_node_handler;
...
void Instance::deInitializeComponents() {
  // Join all threads before destroying all resources
  action_event_manager_.join();
  action_status_manager_.join();
  navigation_event_manager_.join();
  navigation_status_manager_.join();
  status_event_manager_.join();
  query_event_manager_.join();
  agv_order_event_manager_.join();
  control_event_manager_.join();
  factsheet_event_manager_.join();
  interpreter_event_manager_.join();
  message_event_manager_.join();
  order_event_manager_.join();
  state_event_manager_.join();
  validation_event_manager_.join();
...
Instance::Instance(const vda5050pp::Config &config,
    ...
    : config_(config),
      event_manager_options_(event_manager_options),
      action_event_manager_(this->event_manager_options_),
      action_status_manager_(this->event_manager_options_),
      navigation_event_manager_(this->event_manager_options_),
      navigation_status_manager_(this->event_manager_options_),
      status_event_manager_(this->event_manager_options_),
      query_event_manager_(this->event_manager_options_),
      agv_order_event_manager_(this->event_manager_options_),
      control_event_manager_(this->event_manager_options_),
      factsheet_event_manager_(this->event_manager_options_),
      interpreter_event_manager_(this->event_manager_options_),
      message_event_manager_(this->event_manager_options_),
      order_event_manager_(this->event_manager_options_),
      state_event_manager_(this->event_manager_options_),
      validation_event_manager_(this->event_manager_options_) {}

std::weak_ptr<Instance> Instance::init(
  ...
  public:
    explicit InstanceConstructible(
        const vda5050pp::Config &config,
        vda5050pp::core::events::EventManagerOptions event_manager_options) noexcept(true)
        : Instance(config, event_manager_options){};
    ~InstanceConstructible() override = default;
  };

  std::unique_lock i_lock(Instance::instance_mutex_);

  Instance::instance_ = std::make_shared<InstanceConstructible>(config, event_manager_options);
...
OrderEventManager &Instance::getAgvOrderEventManager() noexcept(true) {
  return this->agv_order_event_manager_;
}

...
bool StatusManager::setSafetyState(const vda5050::SafetyState &safety_state) {
  std::unique_lock lock(this->mutex_);
...
const vda5050::SafetyState &StatusManager::getSafetyState() {
  std::shared_lock lock(this->mutex_);  // Ensure that safety is not being altered at the moment
...
std::vector<vda5050::Error> StatusManager::getErrors() const {
  std::shared_lock lock(this->mutex_);
  return this->errors_;
}

...
std::pair<GraphElement::SequenceId, GraphElement::SequenceId> Graph::horizonBounds() const
    noexcept(false) {
  auto match_horz = [](const auto &tup) { return tup.second.isHorizon(); };

  auto it = std::find_if(this->graph_.begin(), this->graph_.end(), match_horz);

  if (it == this->graph_.end()) {
    throw vda5050pp::VDA5050PPInvalidArgument(MK_EX_CONTEXT("Horizon is Empty"));
  }

  return {it->first, this->graph_.rbegin()->first};
}
...
bool Graph::hasOrder() const noexcept(true) {
  if (!this->agv_pos_.has_value()) {
    return false;
  }

  auto [f, l] = this->bounds();

  return *this->agv_pos_ < l;
}
...
std::list<std::shared_ptr<const vda5050::Node>> Graph::getNodes() const {
...
std::list<std::shared_ptr<const vda5050::Edge>> Graph::getEdges() const {
...
std::vector<std::shared_ptr<const vda5050::Node>> Graph::getNodesVec() const {
  decltype(getNodesVec()) ret;
...
std::vector<std::shared_ptr<const vda5050::Edge>> Graph::getEdgesVec() const {
  decltype(getEdgesVec()) ret;

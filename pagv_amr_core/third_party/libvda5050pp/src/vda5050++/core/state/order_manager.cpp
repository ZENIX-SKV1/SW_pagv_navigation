...
void OrderManager::addNewAction(std::shared_ptr<const vda5050::Action> action,
...
std::shared_ptr<const vda5050::Action> OrderManager::getAction(std::string_view action_id) noexcept(
    false) {
...
std::shared_ptr<const vda5050::Action> OrderManager::tryGetAction(std::string_view action_id) {
  auto ret = this->action_by_id_.find(action_id);
  if (ret == this->action_by_id_.end()) {
    return nullptr;
  }
...
std::vector<std::shared_ptr<const vda5050::ActionState>> OrderManager::getActionStates() const {
  std::vector<std::shared_ptr<const vda5050::ActionState>> ret;
  ret.reserve(this->action_state_by_id_.size());

  for (const auto &[_, state] : this->action_state_by_id_) {
    ret.push_back(state);
  }
...
bool OrderManager::replaceGraph(Graph &&new_graph, std::string_view order_id,
                                const std::unique_lock<std::mutex> &lock) noexcept(false) {
  if (this->invalidLock(lock)) {
    throw vda5050pp::VDA5050PPInvalidArgument(MK_EX_CONTEXT("Lock not owned"));
  }
  ...
  return this->setAGVLastNodeId(
      this->graph_->at(0).getId(),
      lock);  // This is almost always equal, only when accepting temporary nodes
              // this mismatches and an "update" of the last node id is required
}

bool OrderManager::replaceGraph(Graph &&new_graph, std::string_view order_id) noexcept(false) {
  std::unique_lock lock(this->mutex_);
  return this->replaceGraph(std::move(new_graph), order_id, lock);
}
...
bool OrderManager::setAGVLastNodeId(std::string_view last_node_id,
                                    const std::unique_lock<std::mutex> &lock) noexcept(false) {
  if (this->invalidLock(lock)) {
    throw vda5050pp::VDA5050PPInvalidArgument(MK_EX_CONTEXT("Lock not owned"));
  }
...
bool OrderManager::setAGVLastNodeId(std::string_view last_node_id) noexcept(false) {
  std::unique_lock lock(this->mutex_);
  return this->setAGVLastNodeId(last_node_id, lock);
}

std::string OrderManager::getAGVLastNodeId() const noexcept(false) {
  std::unique_lock lock(this->mutex_);
  return this->last_node_id_;
}

decltype(vda5050::Node::sequenceId) OrderManager::getAGVLastNodeSequenceId() const noexcept(true) {
  std::unique_lock lock(this->mutex_);
  return this->last_node_sequence_id_;
}
...
void OrderManager::dumpTo(vda5050::State &state) const {
  ...
  state.paused = this->order_status_ == vda5050pp::misc::OrderStatus::k_order_paused ||
                 this->order_status_ == vda5050pp::misc::OrderStatus::k_order_idle_paused ||

...
template <typename T> void sort_inplace(std::vector<T> &vec) {
  static_assert(std::is_same_v<T, vda5050::Node> || std::is_same_v<T, vda5050::Edge>);

  std::sort(vec.begin(), vec.end(), [](T &a, T &b) { return a.sequenceId < b.sequenceId; });
}
...
void MessageEventHandler::handleOrderMessage(
  ...
  sort_inplace(evt->order->nodes);
  sort_inplace(evt->order->edges);
  ...
  if (!vo_res.empty()) {
    ...
    vda5050pp::core::Instance::ref().getMessageEventManager().synchronousDispatch(o_evt);
  }
}
...
void MessageEventHandler::handleInstantActionsMessage(
  ...
  if (!v_res.empty()) {
    getMessagesLogger()->info("InstantActions (headerId={}) contains {} errors: {}",
                              evt->instant_actions->header.headerId, v_res.size(), v_res);
    ...
    vda5050pp::core::Instance::ref().getMessageEventManager().synchronousDispatch(i_evt);
  }
}

...
constexpr auto k_malformed_error = "validationError";  // VDA5050 6.6.4.1
constexpr auto k_semantic_error = "orderError";        // VDA5050 6.6.4.2
constexpr auto k_ia_semantic_error =
    "instantActionError";  // Not specified, inferred from VDA5050 6.6.4.2
...
inline std::variant<T, vda5050::Error> ensureParameterConstraints(
    const vda5050pp::agv_description::ParameterRange &p, const vda5050::json &json,
  ...
  if (p.ordinal_min.has_value()) {
    auto min_value = std::get<T>(*p.ordinal_min);
  ...
  if (p.ordinal_max.has_value()) {
    auto max_value = std::get<T>(*p.ordinal_max);
  ...
  if (p.value_set.has_value() && p.value_set->find(vda5050pp::agv_description::ConstraintValue(
                                     t_value)) == p.value_set->end()) {
    vda5050::Error err;
    err.errorType = context == vda5050pp::misc::ActionContext::k_instant ? k_ia_semantic_error
                                                                         : k_semantic_error;
...
inline std::variant<bool, vda5050::Error> ensureParameterConstraints<bool>(
    const vda5050pp::agv_description::ParameterRange &p, const vda5050::json &value,
    vda5050pp::misc::ActionContext context) {
  bool t_value;
  try {
    value.get_to(t_value);
  } catch (const vda5050::json::exception &e) {
    vda5050::Error err;
    err.errorType = k_malformed_error;
    err.errorDescription = e.what();
...
std::optional<vda5050::Error> doInsert(
    ...
    vda5050pp::misc::ActionContext context) {
  if (auto v = ensureParameterConstraints<T>(decl, p.value, context);
...
std::list<vda5050::Error> vda5050pp::core::checks::contextCheck(
  ...
  vda5050::Error err;
  err.errorType =
      context == vda5050pp::misc::ActionContext::k_instant ? k_ia_semantic_error : k_semantic_error;
  ...
  for (const auto &p : action.actionParameters.value_or(std::vector<vda5050::ActionParameter>())) {
    ...
    switch (decl.type) {
      case vda5050pp::agv_description::ParameterValueType::k_integer:
        maybe_error = doInsert<int64_t>(p, decl, parameters, context);
        break;
      case vda5050pp::agv_description::ParameterValueType::k_float:
        maybe_error = doInsert<double>(p, decl, parameters, context);
        break;
      case vda5050pp::agv_description::ParameterValueType::k_boolean:
        maybe_error = doInsert<bool>(p, decl, parameters, context);
        break;
      case vda5050pp::agv_description::ParameterValueType::k_string:
        maybe_error = doInsert<std::string>(p, decl, parameters, context);
        break;
      case vda5050pp::agv_description::ParameterValueType::k_array:
        throw vda5050pp::VDA5050PPInvalidConfiguration(
            MK_FN_EX_CONTEXT("Arrays are not supported with the default validation."));
      case vda5050pp::agv_description::ParameterValueType::k_object:
        throw vda5050pp::VDA5050PPInvalidConfiguration(
            MK_FN_EX_CONTEXT("Objects are not supported with the default validation."));
      default:
        throw vda5050pp::VDA5050PPInvalidConfiguration(
...
std::list<vda5050::Error> vda5050pp::core::checks::uniqueActionId(
    const vda5050::Action &action, std::set<std::string_view, std::less<>> &seen,
    const vda5050pp::misc::ActionContext context) noexcept(false) {
  if (common::contains(seen, action.actionId)) {
    vda5050::Error error;
    error.errorType = context == vda5050pp::misc::ActionContext::k_instant ? k_ia_semantic_error
                                                                           : k_semantic_error;
...
std::list<vda5050::Error> vda5050pp::core::checks::controlActionFeasible(
  ...
  if (action.actionType == "startPause" && !isPauseable(order_status)) {
    vda5050::Error error;
    error.errorType = k_ia_semantic_error;

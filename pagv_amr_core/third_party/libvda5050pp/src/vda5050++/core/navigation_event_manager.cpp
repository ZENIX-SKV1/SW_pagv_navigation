...
void ScopedNavigationEventSubscriber::subscribe(
    std::function<void(std::shared_ptr<vda5050pp::events::NavigationReset>)>
        &&callback) noexcept(true) {
  this->remover_.appendListener(
      vda5050pp::events::NavigationEventType::k_reset,
      eventpp::argumentAdapter<void(std::shared_ptr<vda5050pp::events::NavigationReset>)>(
          std::move(callback)));
}

NavigationEventManager::NavigationEventManager(
    const vda5050pp::core::events::EventManagerOptions &opts)
    : opts_(opts),
...
void NavigationEventManager::dispatch(
    std::shared_ptr<vda5050pp::events::NavigationHorizonUpdate> data,
    bool synchronized_dispatch) noexcept(true) {
  if (this->opts_.synchronous_event_dispatch || synchronized_dispatch) {
...
void NavigationEventManager::dispatch(
    std::shared_ptr<vda5050pp::events::NavigationBaseIncreased> data,
    bool synchronized_dispatch) noexcept(true) {
  if (this->opts_.synchronous_event_dispatch || synchronized_dispatch) {
...
void NavigationEventManager::dispatch(std::shared_ptr<vda5050pp::events::NavigationNextNode> data,
                                      bool synchronized_dispatch) noexcept(true) {
  if (this->opts_.synchronous_event_dispatch || synchronized_dispatch) {
...
void NavigationEventManager::dispatch(
    std::shared_ptr<vda5050pp::events::NavigationUpcomingSegment> data,
    bool synchronized_dispatch) noexcept(true) {
  if (this->opts_.synchronous_event_dispatch || synchronized_dispatch) {
...
void NavigationEventManager::dispatch(std::shared_ptr<vda5050pp::events::NavigationControl> data,
                                      bool synchronized_dispatch) noexcept(true) {
  if (this->opts_.synchronous_event_dispatch || synchronized_dispatch) {
...
void NavigationEventManager::dispatch(std::shared_ptr<vda5050pp::events::NavigationReset> data,
                                      bool synchronized_dispatch) noexcept(true) {
  if (this->opts_.synchronous_event_dispatch || synchronized_dispatch) {
    this->navigation_event_queue_.dispatch(vda5050pp::events::NavigationEventType::k_reset, data);
  } else {
    this->navigation_event_queue_.enqueue(vda5050pp::events::NavigationEventType::k_reset, data);
  }
}

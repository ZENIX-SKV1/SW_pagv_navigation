...
std::optional<std::list<vda5050::Error>> BaseQueryHandler::callQueryAcceptZoneSet(
  ...
  std::unique_lock lock(this->call_query_accept_zone_set_mutex_);
...
std::optional<std::list<vda5050::Error>> BaseQueryHandler::callQueryNodeTriviallyReachable(
    std::shared_ptr<const vda5050::Node> node) {
  std::unique_lock lock(this->call_query_node_trivially_reachable_mutex_);
  this->overwrite_query_node_trivially_reachable_ = true;

  if (auto result = this->queryNodeTriviallyReachable(node);
      this->overwrite_query_node_trivially_reachable_) {
    return result;
  } else {
    return std::nullopt;
  }
...
std::list<vda5050::Error> BaseQueryHandler::queryNodeTriviallyReachable(
    std::shared_ptr<const vda5050::Node>) {
  this->overwrite_query_node_trivially_reachable_ = false;
  return {};
}
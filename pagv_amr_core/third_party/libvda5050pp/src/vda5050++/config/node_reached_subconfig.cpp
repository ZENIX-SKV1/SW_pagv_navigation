...
void NodeReachedSubConfig::getFrom(const ConstConfigNode &node) {
  ...
  this->default_node_deviation_xy_ = node_view["default_node_deviation_xy"].value<double>();
  this->default_node_deviation_theta_ = node_view["default_node_deviation_theta"].value<double>();
...
void NodeReachedSubConfig::putTo(ConfigNode &node) const {
  ...
  if (this->default_node_deviation_xy_) {
    table->insert("default_node_deviation_xy", *this->default_node_deviation_xy_);
  }
  if (this->default_node_deviation_theta_) {
...
void NodeReachedSubConfig::setDefaultNodeDeviationXY(std::optional<double> new_value) {
  this->default_node_deviation_xy_ = new_value;
}
void NodeReachedSubConfig::setDefaultNodeDeviationTheta(std::optional<double> new_value) {
...
std::optional<double> NodeReachedSubConfig::getDefaultNodeDeviationXY() const {
  return this->default_node_deviation_xy_;
}
std::optional<double> NodeReachedSubConfig::getDefaultNodeDeviationTheta() const {

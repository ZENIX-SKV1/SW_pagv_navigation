...
void StatusSink::setSafetyState(const vda5050::SafetyState &safety_state) const noexcept(false) {
  auto event = std::make_shared<vda5050pp::events::SafetyStateSet>();
  event->safety_state = safety_state;

  vda5050pp::core::Instance::ref().getStatusEventManager().dispatch(event);
}

vda5050::SafetyState StatusSink::getSafetyState() const noexcept(false) {
  auto event = std::make_shared<vda5050pp::events::SafetyStateGet>();
  auto future = event->getFuture();
...
void StatusSink::alterSafetyState(
    std::function<void(vda5050::SafetyState &)> &&alter_function) const noexcept(false) {
  auto event = std::make_shared<vda5050pp::events::SafetyStateAlter>();
  event->alter_function = std::move(alter_function);
